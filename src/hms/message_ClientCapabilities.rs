...
pub struct ClientCapabilities {
    pub values: ::std::vec::Vec<ClientCapability>,
}
impl ::pilota::thrift::Message for ClientCapabilities {
        ...
        if let ::std::result::Result::Err(mut err) = (|| {
                ...
                match field_ident.id {
                    Some(1) if field_ident.field_type == ::pilota::thrift::TType::List => {
                        var_1 = Some(unsafe {
                            ...
                            let mut val: ::std::vec::Vec<ClientCapability> =
            ...
            if let Some(field_id) = __pilota_decoding_field_id {
                err.prepend_msg(&format!(
                    "decode struct `ClientCapabilities` field(#{}) failed, caused by: ",
                    field_id
                ));
            }
    ...
    > {
        ::std::boxed::Box::pin(async move {
            ...
            if let ::std::result::Result::Err(mut err) = async {
                ...
                match field_ident.id {
                    ...
                    var_1 = Some({
                            ...
                            for _ in 0..list_ident.size {
                                val.push(<ClientCapability as ::pilota::thrift::Message>::decode_async(__protocol).await?);
                            };
                ...
                if let Some(field_id) = __pilota_decoding_field_id {
                    err.prepend_msg(&format!("decode struct `ClientCapabilities` field(#{}) failed, caused by: ", field_id));
                }
                return ::std::result::Result::Err(err);
            };
