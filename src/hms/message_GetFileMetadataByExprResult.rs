...
pub struct GetFileMetadataByExprResult {
    pub metadata: ::pilota::AHashMap<i64, MetadataPpdResult>,

    pub is_supported: bool,
}
impl ::pilota::thrift::Message for GetFileMetadataByExprResult {
        ...
        if let ::std::result::Result::Err(mut err) = (|| {
            ...
            if let Some(field_id) = __pilota_decoding_field_id {
                err.prepend_msg(&format!(
                    "decode struct `GetFileMetadataByExprResult` field(#{}) failed, caused by: ",
                    field_id
                ));
            }
            ...
            if let ::std::result::Result::Err(mut err) = async {
                ...
                match field_ident.id {
                    ...
                    var_1 = Some({
                        ...
                        let mut val = ::pilota::AHashMap::with_capacity(map_ident.size);
                        for _ in 0..map_ident.size {
                            val.insert(__protocol.read_i64().await?, <MetadataPpdResult as ::pilota::thrift::Message>::decode_async(__protocol).await?);
                        }
                ...
                if let Some(field_id) = __pilota_decoding_field_id {
                    err.prepend_msg(&format!("decode struct `GetFileMetadataByExprResult` field(#{}) failed, caused by: ", field_id));
                }
                return ::std::result::Result::Err(err);
            };
