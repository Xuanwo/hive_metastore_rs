...
pub struct PartitionValuesRow {
    pub row: ::std::vec::Vec<::pilota::FastStr>,
}
impl ::pilota::thrift::Message for PartitionValuesRow {
    ...
    fn decode<T: ::pilota::thrift::TInputProtocol>(
        ...
        if let ::std::result::Result::Err(mut err) = (|| {
            ...
            if let Some(field_id) = __pilota_decoding_field_id {
                err.prepend_msg(&format!(
                    "decode struct `PartitionValuesRow` field(#{}) failed, caused by: ",
                    field_id
                ));
            }
        ...
        let Some(var_1) = var_1 else {
            return ::std::result::Result::Err(::pilota::thrift::new_protocol_exception(
                ...
                "field row is required".to_string(),
            ));
        };

        let data = Self { row: var_1 };
