...
pub struct PrivilegeGrantInfo {
    pub privilege: ::std::option::Option<::pilota::FastStr>,
...
impl ::pilota::thrift::Message for PrivilegeGrantInfo {
    fn encode<T: ::pilota::thrift::TOutputProtocol>(
        ...
        if let Some(value) = self.privilege.as_ref() {
        ...
        if let ::std::result::Result::Err(mut err) = (|| {
            ...
            if let Some(field_id) = __pilota_decoding_field_id {
                err.prepend_msg(&format!(
                    "decode struct `PrivilegeGrantInfo` field(#{}) failed, caused by: ",
                    field_id
                ));
            }
